...
public class StrategyArmory implements IStrategyArmory {
    ...
    public boolean assembleLotteryStrategy(Long strategyId) {
        ...
        List<StrategyArmoryEntity> strategyArmoryEntities = repository.queryStrategyAwardList(strategyId);
        //获取最小概率
        BigDecimal minAwardRate = strategyArmoryEntities.stream()
        ...
        //获取概率值的总和
        BigDecimal totalAwardRate = strategyArmoryEntities.stream()
                .map(strategyArmoryEntity -> strategyArmoryEntity.getAwardRate())
        ...
        //构造概率表格
        for (StrategyArmoryEntity strategyArmoryEntity : strategyArmoryEntities){

            for (int i=0; i<rangeAwardRate.multiply(strategyArmoryEntity.getAwardRate()).setScale(BigDecimal.ROUND_CEILING).intValue();i++){
                //将奖品Id放在概率表中，如果概率表中该奖品Id出现的次数越多，则表示该奖品的概率越大
                strategyAwardSerachRateTable.add(strategyArmoryEntity.getAwardId());
            }
            log.info("奖品Id:{}，在概率表中出现次数为：{}",strategyArmoryEntity.getStrategyId(),strategyAwardSerachRateTable.size());
        }
